...
function scheduleEvents(events){
    for (let i = 0; i<events.size(); i++){
        ...
        play(note.channel, note.pitch, note.vel, start_time_seconds + beat_time_to_real_seconds(note.start) + LEAD_TIME_MILLIS/1000.0, beat_time_to_real_seconds(note.dur))
    }
}
...
async function masterLoop(){
    ...
    const wakeup_time_s = start_time_seconds + beat_time_to_real_seconds(current_time_beats + rest_length) - LEAD_TIME_MILLIS/1000 //preempt the wakeup time by 250s
