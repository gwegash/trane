...
async function createParameters() {
  ...
  await Promise.all(
    (await readdir("./ui")).map(async (file) => {
      if (file.endsWith(".ts")) {
        const parsed = await parser.parseFile(`./ui/${file}`, "workspace root")
        const classDeclarations = parsed.declarations.filter(
          (declaration) => declaration instanceof ClassDeclaration,
        )
        await Promise.all(
          classDeclarations.map(async (classDeclaration) => {
            const paramsDef = classDeclaration.properties.find(
              (property) => property.name === "params",
            )
            const friendlyNameDef = classDeclaration.properties.find(
              (property) => property.name === "friendlyName",
            )
            if (paramsDef && friendlyNameDef) {
              //Ok, this looks good. let's open the file
              const contents = await readFile(`./ui/${file}`)

              const matches = rx.exec(
                contents.subarray(paramsDef.start, paramsDef.end).toString(),
              )
              const friendlyName = contents
                .subarray(friendlyNameDef.start, friendlyNameDef.end)
                .toString()
                ?.split('"')
                ?.at(1)
                ?.trim()
  ...
  if (Object.values(paramsMap).length > 0) {
    janetFormattedParamsMap = Object.entries(paramsMap)
      .map(
        ([friendlyName, params]) =>
          `    :${friendlyName} @{\n` +
          params.map((param, idx) => `      :${param.name} ${idx}\n`).join("") +
          "    }\n",
      )
      .join("")
    ...
    console.error(
      "something went wrong trying to generate parameter definitions",
    )
  }
}
